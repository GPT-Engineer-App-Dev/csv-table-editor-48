...
export const useAddShoppingCart = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (newCart) => fromSupabase(supabase.from('shopping_cart').insert([newCart])),
        onSuccess: () => {
...
export const useUpdateShoppingCart = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (updatedCart) => fromSupabase(supabase.from('shopping_cart').update(updatedCart).eq('id', updatedCart.id)),
        onSuccess: () => {
            queryClient.invalidateQueries('shopping_cart');
            queryClient.invalidateQueries(['shopping_cart', updatedCart.id]);
        },
    });
};
